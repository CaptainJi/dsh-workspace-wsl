...
Registers the `dsh_workspace_wsl` tool and renders its call/result as a **diff card** via
...
dsh plugin --profile my-profile add ./@deepseek-ai/dsh-wsl-workspace
dsh --profile my-profile       # watch for: [deepseek-ai-dsh-wsl-workspace] registered "dsh_workspace_wsl" (webui diff card)
```

## Dependencies pinned / 依赖锁定

- `@deepseek-ai/dsh-tools`: `0.1.0-rc.6` (exact — the **`next`**-tag line).
- `@deepseek-ai/cordis`: `^4.0.1` (peerDependency — host provides it).
