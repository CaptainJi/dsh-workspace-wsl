...
export async function apply(ctx: any) {
  ...
  const setOpen = (v: boolean) => { dialogOpen = !!v; for (const fn of Array.from(subs)) { try { fn() } catch {} } }
  ...
  // Adapter: when the built-in add flow opens, withdraw it and open our dialog.
  ...
  function WorkspaceDialog() {
    ...
    async function confirmWsl() {
      ...
      const payload: any = { kind: 'wsl', distro: d, wslPath: p }
    ...
    function renderItems() {
      ...
      const group = (title: string, arr: any[]) => arr.length === 0 ? null : el('div', null,
      ...
      body = el('div', null,
        ...
        useAuto
          ? el('select', { className: 'wslwk-select', value: distro, disabled: busy, onChange: (e) => { const d = e.target.value; setDistro(d); setWslPath('/'); setEntries(null); loadDir(d, '/') } },
              probe.distros.map((d: string) => el('option', { key: d, value: d }, d)))
